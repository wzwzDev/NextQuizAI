...
function SignInInner() {
  ...
  const handleUserCredentialsSignIn = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setUserError(null);
    ...
    try {
      ...
      if (!result || result.error) {
        setUserError("Invalid email/password or email not verified yet.");
        return;
      }
      ...
      setUserError("Sign-in failed. Please try again.");
    } finally {
    ...
    <main className="relative mx-auto flex min-h-screen w-full max-w-6xl items-center px-4 py-10 sm:px-8">
      ...
      <div className="grid w-full gap-6 md:grid-cols-2">
        <section className="section-shell rounded-2xl p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Continue with your regular account.</p>
          ...
          <form className="space-y-4" onSubmit={handleUserCredentialsSignIn}>
            <div className="space-y-1">
              <label htmlFor="user-email" className="text-sm font-semibold text-foreground">
                Email
              </label>
              <input
                id="user-email"
            ...
            <div className="space-y-1">
              <label htmlFor="user-password" className="text-sm font-semibold text-foreground">
                Password
              </label>
            ...
            >
              {isSubmittingUser ? "Signing in..." : "Sign in with Email"}
            </button>
